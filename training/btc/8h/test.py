...
str_time_horizon = config.str_time_horizon                 # 30min, 1h, 2h, 3h, 4h, 6h, 8h, 12h, 24h 
utc = 0

training_start_date = config.training_start_date       # must be greator than 2012-01-01
backtest_duration = config.backtest_duration                  # in days
forwardtest_duration = config.forwardtest_duration                # in days

transaction_fee_percent = config.transaction_fee_percent           # transaction fee in percentage 0.0, 0.025, 0.05, ...
take_profit_percent = config.take_profit_percent                  # take profit in percentage 1, 2, 3, ... 
stop_loss_percent = config.stop_loss_percent                    # stop loss in percentage 1, 2, 3, ...
leverage = config.leverage                             # leverage for the instrument 1x, 2x, 3x, ...
...
for model_path in list_of_models:
	...
	df_best_ledger.to_csv(os.path.join(models_directory, f'ledger_{model_name_prefix}.csv'))
